...
object subtyping {
  ...
  def observeCounterList (f : List[Counter] => Unit) : List[Int] = {
    ...
    val Counter_List = List(new Counter1, new Counter1, new Counter1)
	f(Counter_List)
	Counter_List.map((x:Counter1)=>x.count)
  }
  ...
  def observeCounterArray (f : Array[Counter] => Unit) : Array[Int] = {
    // TODO: Provide definition here.
    val First_Counter = new Counter1
	val Second_Counter = new Counter1
	val Third_Counter = new Counter1
	val Counter_Array = List(First_Counter.asInstanceOf[Counter],Second_Counter.asInstanceOf[Counter],Third_Counter.asInstanceOf[Counter]).toArray
	f(Counter_Array)
	List(Counter_Array(0).asInstanceOf[Counter1].count, Counter_Array(1).asInstanceOf[Counter1].count, Counter_Array(2).asInstanceOf[Counter1].count).toArray
  }
}
