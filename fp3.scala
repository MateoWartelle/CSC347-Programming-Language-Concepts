...
object fp3 {
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    // TODO: Provide definition here.
     def removeDupes2Aux [X] (x:X, ys:List[X], i:Int) :List[(Int, X)] = {
        ys match{
          case Nil    =>    if (x != Nil) (i, x)::List() else List()
          case y::ys  => {  if (y != x) (i, x) :: removeDupes2Aux(y,ys,1) 
                            else removeDupes2Aux(y,ys,i+1)
        }
      }
    }
    if (xs.length>0) removeDupes2Aux(xs.head, xs.tail, 1)
    else List()
  }
